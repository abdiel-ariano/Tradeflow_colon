...
# AWS S3 media (preferred on the AWS deployment). The EC2 instance profile
# supplies short-lived credentials; never configure access keys in runtime.env.
AWS_MEDIA_BUCKET_NAME = config('AWS_MEDIA_BUCKET_NAME', default='').strip()
AWS_MEDIA_REGION_NAME = config('AWS_MEDIA_REGION_NAME', default='us-east-1').strip()
AWS_MEDIA_URL_EXPIRY = config('AWS_MEDIA_URL_EXPIRY', default=3600, cast=int)
...
# Storage default: filesystem in local development; WhiteNoise for static files.
STORAGES = {
    'default': {
...
if AWS_MEDIA_BUCKET_NAME:
    ...
    STORAGES['default'] = {
        ...
        'OPTIONS': {
            ...
            'region_name': AWS_MEDIA_REGION_NAME,
            # BucketOwnerEnforced rejects object ACLs. Access stays private;
            # browser reads use short-lived SigV4 URLs generated by Django.
            'default_acl': None,
            'file_overwrite': False,
            'querystring_auth': True,
            'querystring_expire': AWS_MEDIA_URL_EXPIRY,
...
elif SUPABASE_SERVICE_KEY and SUPABASE_URL:
    if 'storages' not in INSTALLED_APPS:
        INSTALLED_APPS.append('storages')
    STORAGES['default'] = {
