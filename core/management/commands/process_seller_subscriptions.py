...
=============================================================================
Job diario de ciclo de vida SaaS seller (trial → gracia → baja media).

EJECUCIÓN
---------
    python manage.py process_seller_subscriptions

Programar en Railway/cron una vez al día (UTC recomendado: 06:00).

LÓGICA IDEMPOTENTE
------------------
1. ``trialing`` con ``current_period_end < now`` → ``finalize_trial_period()``
2. ``past_due`` con ``grace_ends_at < now`` → ``apply_medium_churn()``
3. Emails opcionales de recordatorio de gracia (días 33 y 36 aprox.)
...
class Command(BaseCommand):
    help = 'Procesa vencimientos de trial y gracia de suscripciones seller.'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(
            f'process_seller_subscriptions: finalized={finalized} churned={churned} reminders={reminders}',
        ))
