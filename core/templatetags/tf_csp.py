"""
Django template tags — CSP nonce helpers for inline JSON blocks.

Usage::

    {% load tf_csp %}
    {% json_data_block tf_i18n "tf-i18n-data" %}

Renders ``<script type="application/json" id="..." nonce="...">`` with JSON
escaped like Django's ``json_script`` plus the CSP nonce from ``request.csp_nonce``.
"""
from __future__ import annotations

import json

from django import template
from django.utils.html import format_html
from django.utils.safestring import mark_safe

register = template.Library()

# Escapes identical to Django.utils.html.json_script so JSON with </ or <!--
# cannot break out of the script block.
_JSON_SCRIPT_ESCAPES = {
    ord('>'): '\\u003E',
    ord('<'): '\\u003C',
    ord('&'): '\\u0026',
}


@register.simple_tag(takes_context=True)
def json_data_block(context, value, element_id: str):
    """Like ``json_script`` but adds the CSP nonce attribute."""
    request = context.get('request')
    nonce = getattr(request, 'csp_nonce', '') if request is not None else ''
    encoded = json.dumps(value, separators=(',', ':')).translate(_JSON_SCRIPT_ESCAPES)
    return format_html(
        '<script id="{}" type="application/json" nonce="{}">{}</script>',
        element_id, nonce, mark_safe(encoded)
    )
